...
## 首次实验结果（2026-06-17，run `exp_20260617_190222_llm`）

7 轮后早停（patience=3），探索 314 个因子，14 次调用 / 5.9 万 token / ¥0.24。

**单库 OOS 盖章（screen_factors，库 N=314）**：

- winner `scale(mul(close,decay_linear(volume,5)))`：train_ic −0.0546 / test_ic −0.0524。
- deflated：测试段 `|NW_t|=4.09 > E[max t]@314=2.91` → **✓ 过**，`p=0.0134`。
- winner OOS 回测（含 10bp 手续费）：多头 T+1 夏普 **0.92**、年化 **+5.87%**。
- 全库 `train_ic vs test_ic` 秩相关 **rho=+0.718** → 样本内能预测样本外，搜索机制有效。

**横向对比（compare_generators，统一最严 N=7262，门槛 3.78）**：

| 生成器 | N | rho | ceiling | win\|NWt\| | deflated | size |
| --- | --- | --- | --- | --- | --- | --- |
| 随机动物园\* | 80 | −0.453 | 0.033 | 1.01 | ✗ | 5.5 |
| RL | 7262 | +0.856 | 0.0666 | 2.58 | ✗ | 8.2 |
| **LLM** | **314** | +0.718 | 0.0546 | **4.09** | **✓** | **6.3** |

\* 这条是跨 regime 的旧冒烟库（N=80，负 rho），非合法 within-regime 基线，仅占位；正确的 N=1730 库本机暂缺。

**结论**：

- **LLM 是统一最严门槛下唯一过 deflated 的生成器**——winner OOS 更强（4.09 vs RL 2.58）、更简洁（6.3 vs 8.2），且只搜了 1/23（314 vs 7262）。
- **deflated 税被量化坐实**：RL 双重失败——winner 本就更弱，又因搜 7262 次把门槛垫到 3.78 够不着。**搜得多 ≠ 找得好**（见 [因子动物园](因子动物园.md)）。
- **但三者 ceiling 都在 0.03~0.067，收敛到同一 amount/流动性天花板家族**：LLM 没突破价量，只是更高效、OOS 更干净地找到了那个天花板因子。309 个因子坍缩成≈一个经济直觉。破天花板仍需新数据。
